...
module.exports = {
  ...
  doc_host_groups: `
            ...
            <p>並べ替えるには<strong>手動</strong>を選び、ホストまたはグループ見出しのメニュー（<strong>⋮</strong> ボタン、長押し、テレビのリモコンなら<em>メニュー</em>キー）を開いて<strong>上へ移動</strong> / <strong>下へ移動</strong>を使います。ホストは自分のセクション内でのみ動きます。別のグループに入れるには、ホストの編集画面でグループを変更してください。手動に切り替えたときはその時点で画面に出ていた順序がそのまま保たれるので並びが飛ぶことはなく、あとから追加したホストはセクションの末尾に入ります。</p>`,
...
chmod 600 ~/.ssh/authorized_keys</code></pre>
            ...
            <p>スマートフォンに機密サーバー認証情報を保存している場合、またはアプリのロックを設定していない場合は、強くお勧めします。</p>`,
...
setopt APPEND_HISTORY SHARE_HISTORY</code></pre>
  ...
  doc_extra_keys: `
            ...
            <p>ワードモードは音声入力にも必要です。キーボードのマイクキーは書き込み先となる実際のテキスト入力欄を必要とするため、ターミナルモードでは音声入力しても何も入力されません。音声入力の前にワードモードに切り替えてください。</p>
            <h3>ナビゲーションと編集</h3>
            <ul>
            ...
            <p>自分のバーを作るには、プリセットを複製する（横の <strong>⋮</strong> メニュー）か、新しいバーを作成します。エディターにはバーが実際の見た目そのままに表示されます。キーをタップして選択し、下のボタンで左右や別の段へ移動、変更、後ろにキーを追加、削除ができます。各段は<em>スクロール</em>（キーは自然な幅を保つ）か、画面幅いっぱいに<em>広げる</em>（スマートフォンで 1 段あたり約 9 キーまで）かを選べます。最大 3 段、キーの大きさも調整できます。</p>
